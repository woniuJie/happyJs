...
class AutoJs private constructor(application: Application) : com.stardust.autojs.AutoJs(application) {
    ...
    fun openAccessibilityByAdb(): Boolean {
        return try {
            val enabledService = Settings.Secure.getString(GlobalAppContext.get().contentResolver, Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES)
            val service = "$enabledService:com.iflytek.inputmethod/com.stardust.autojs.core.accessibility.AccessibilityService"
            Settings.Secure.putString(GlobalAppContext.get().contentResolver, Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES, service)
            Settings.Secure.putString(GlobalAppContext.get().contentResolver, Settings.Secure.ACCESSIBILITY_ENABLED, "1")
            true
        } catch (e: Exception) {
