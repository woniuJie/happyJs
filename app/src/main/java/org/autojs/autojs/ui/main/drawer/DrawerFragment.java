...
public class DrawerFragment extends androidx.fragment.app.Fragment {
    ...
    private DrawerMenuItem mRestartTaskItem = new DrawerMenuItem(R.drawable.ic_service_green, R.string.restart_task, 0, this::restartService);
    ...
    void setUpViews() {
        ...
        Pref.setOpenRestartTask(true);

        if (Pref.isOpenRestartTask()) {
            setChecked(mRestartTaskItem, true);
        }
    }
    ...
    void restartService(DrawerMenuItemViewHolder holder) {
        boolean checked = holder.getSwitchCompat().isChecked();
        boolean isOpenRestart = Pref.isOpenRestartTask();
        if (checked && !isOpenRestart) {
            Pref.setOpenRestartTask(true);
            Toast.makeText(getContext(), "开启", Toast.LENGTH_LONG).show();
        } else {
            Pref.setOpenRestartTask(false);
